...
! icode=1 for I1 (binomial thinning), icode=2 for I2
...
subroutine ebinom(xmx,icode,param,cdfvec,pmfvec)
  ...
  double precision I1gkxchf, I2gkxchf
  double precision param(3), cdfvec(xmx+1),pmfvec(xmx+1),xprev
  external I1gkxchf, I2gkxchf
  select case(icode)
  case(1)
...
! can add other operators here such as
! chf for I3 operator with compounding  sum_{j=0}^xprev K_j(x;alpha,gamma)
